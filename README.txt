...
Using the Raspberry Pi OS installer (https://www.raspberrypi.com/software/) and a blank microSD card (128 GB Recommended) write the raspbian OS onto the SD card. Under the "operating system" menue select "Rapberry Pi OS (other)" and select "Raspberry Pi OS Full (32-bit)". Once operating system is writen to the SD card place into Raspberry Pi. 

Once initial set up is completed go to terminal (Ctrl + T) and run the following codes:
	sudo apt update
...
System will require a reboot following these changes. The final items that need to be installed are all the python packaged and adafruit system requirments for CircuitPython. Tutorials and information can be found on Adafruit's website (https://learn.adafruit.com/welcome-to-circuitpython).
...
Upack the files by unzipping and move RunChamber.sh file to the desktop. 
